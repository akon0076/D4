...
public class CaptchaHelper {
    ...
    private void entry(HttpServletRequest request, HttpServletResponse response, String id) throws Exception {
        ...
        g.setColor(color);
        //设置字体
        InputStream ttfFile = CaptchaHelper.class.getResourceAsStream("/captcha.ttf");
     ...
     * @param captcha 输入的验证码的值
     */
    public void validateThrow(HttpServletRequest request, String captcha) {
        if (!captchaHelper.verify(request, captcha)) {
            throw new DDDException("验证码错误或已过期");
        }
    }
}